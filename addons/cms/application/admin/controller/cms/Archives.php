...
class Archives extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $params = array(
                ...
                'archive_id'=>array('IN',$ids)
            );
            
            $modelRecommend = new \app\admin\model\cms\ArchivesRecommend;
            $row = $modelRecommend->where($params)->select();

            foreach($row as $item){
                $tmp[$item['archive_id']]['recommend'] = true;
            }
    ...
    public function recommend($ids = NULL){
        if ($this->request->isAjax()) {
            ...
            $row = $modelRecommend->get($params);
            if (!$row) {
                $modelRecommend->save($params);
            }
            $this->success();
        }
    }
    public function unrecommend($ids = NULL){
        if ($this->request->isAjax()) {
            $admin_id = $this->auth->id;
            ...
            $modelRecommend->where($params)->delete();
            $this->success();
        }
    }
