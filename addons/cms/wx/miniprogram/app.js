...
App({
  ...
  apiUrl: 'https://az-ips.wxhoutai.com/addons/cms/wxapp.',
  // apiUrl: 'http://az-ips.com/addons/cms/wxapp.',
  si: 0,
  //小程序启动
  onLaunch: function() {
    var that = this;
  ...
  //action
  action: function(event,cb) {
    var that = this;
    var id = event.currentTarget.dataset.id;
    var action = event.currentTarget.dataset.action;
    var actionData = wx.getStorageSync(action) || [];
    if (actionData.indexOf(id) > -1) {
      that.info("请勿重复操作");
      return;
    }
    actionData.push(id);
    wx.setStorageSync(action, actionData);
    this.request('/archives/action', {
      archive_id: id,
      action: action
    }, function(data, ret) {
  ...
  //登录
  loginByMobile: function(mobile,cb) {
    var that = this;
    var token = wx.getStorageSync('token') || '';

    wx.request({
      url: that.apiUrl + 'user/loginByMobile',
      data: {
        mobile: mobile
      },
      method: 'post',
