...
class Archives extends Model
{
     ...
     * @return array|false|\PDOStatement|string|\think\Collection
     */
    public static function getRecommendArchivesList($tag,$admin_ids)
    {
        $config = get_addon_config('cms');
        $type = empty($tag['type']) ? '' : $tag['type'];
        $model = !isset($tag['model']) ? '' : $tag['model'];
        ...
        $modelRecommend = new \app\admin\model\cms\ArchivesRecommend;
        $_recommendList = collection($modelRecommend->where($_params)->field('archive_id')->select())->toArray();
        $_recommendIds = array();
        foreach($_recommendList as $v){
            $_recommendIds[] = $v['archive_id'];
        }
        //
        $list = $archivesModel
            ->where($where)
            ->where($condition)
            ->where(array('id'=> array('IN' , $_recommendIds)))
            ->field($field)
            ->cache($cache)
            ->order($order)
            ->limit($limit)
